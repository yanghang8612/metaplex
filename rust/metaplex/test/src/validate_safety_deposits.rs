...
pub fn validate_safety_deposits(app_matches: &ArgMatches, payer: Keypair, client: RpcClient) {
    ...
    let configs_to_validate: Vec<WinningConfig>;

    if winner_config_slot == -1 {
        configs_to_validate = manager.settings.winning_configs;
    } else {
        configs_to_validate = vec![manager.settings.winning_configs[winner_config_slot as usize]];
    }

    for n in 0..configs_to_validate.len() {
        let config = configs_to_validate[n];
        let (config_box, box_key) = safety_deposits
            .get(&config.safety_deposit_box_index)
            .unwrap();
